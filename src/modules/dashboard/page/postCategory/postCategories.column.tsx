...
import { PostCategoryItems } from "../../types/post-categories.type";

export default function PostCategoryColumnsProvider() {
  ...
  const getPostCategoriesColumns = (): ColumnDef<PostCategoryItems>[] => [
    {
      accessorKey: "title",
      header: ({ column }) => {
        return (
            ...
            <DropdownMenuContent align="end">
              ...
              {category.isActive ? (
                <DropdownMenuItem
                  onClick={() =>
                    publicAxios
                      .put(`/posts/category/${category._id}/deactivate`)
                      .then((data) => {
                        toast.success(data.data.message);
                ...
                <DropdownMenuItem
                  onClick={() =>
                    publicAxios
                      .put(`/posts/category/${category._id}/activate`)
                      .then((data) => {
                        toast.success(data.data.message);
                        router.refresh(); // Refresh the page to show updated data
                      })
  ...
  return { getPostCategoriesColumns };
}

// Export the original function for backward compatibility
export const getPostCategoriesColumns = (): ColumnDef<PostCategoryItems>[] => {
  // Create an instance of the component to access its methods
  const { getPostCategoriesColumns: getColumns } =
    PostCategoryColumnsProvider();
  return getColumns();
};
